        ...
        for column, col in enumerate(rows):
            ...
            if setting_column:
                ...
                if setting_column.type == 'datetime':
                    if value != '':
                        value = str(datetime.datetime(*xldate_as_tuple(xldate_from_datetime_tuple(eval(value), 0), 0)))
                    else:
                        value = None
...
def main():
    ...
    fh = open(os.devnull,'w') #WARNING *** XF[167] unknown (raw) format key (56, 0x0038) が邪魔臭いので
    wb = open_workbook(input_filename, logfile=fh)
    fh.close()
